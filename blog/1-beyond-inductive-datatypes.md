...
inductive types (HITs) and the cubical path type. I'm a simpler man, so I just
call this "smart constructors". The concept is implemented in Arend with a nice
syntax, and the idea was invented individually by you and the language authors.  
...
returning two different types. When `len` zero, `Vector` becomes a datatype only
