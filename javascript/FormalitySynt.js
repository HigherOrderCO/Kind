...
function typecheck(term, type, defs, show = stringify, hols = {}, ctx = Nil(), locs = null) {
  ...
  switch (term.ctor) {
    ...
    case "Hol":
      // If we try to type-check a hole and it is of type `(x : A) -> B`, we can
      // specialize it into a function `(x) ?` and recurse.
      if (typv.ctor === "All") {
        var nam0 = new_name();
        var hols = {...hols, [term.name]: (vals) => {
          return Lam(typv.eras, typv.name || ("x"+ctx.size), x => Hol(nam0, Ext(x, vals)));
        }};
        return deep([[typecheck, [hols[term.name](term.vals), type, defs, show, hols, ctx, locs]]], ([hols, _]) => {
          return done([hols, type]);
        });
