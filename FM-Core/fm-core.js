...
const Var = (index)                        => Lazy(["Var", {index}]);
const Typ = ()                             => Lazy(["Typ", {}]);
const All = (name, bind, body, eras)       => Lazy(["All", {name, bind, body, eras}]);
const Lam = (name, bind, body, eras)       => Lazy(["Lam", {name, bind, body, eras}]);
const App = (func, argm, eras)             => Lazy(["App", {func, argm, eras}]);
const Box = (expr)                         => Lazy(["Box", {expr}]);
const Put = (expr)                         => Lazy(["Put", {expr}]);
const Dup = (name, expr, body)             => Lazy(["Dup", {name, expr, body}]);
const U32 = ()                             => Lazy(["U32", {}]);
const Num = (numb)                         => Lazy(["Num", {numb}]);
const Op1 = (func, num0, num1)             => Lazy(["Op1", {func, num0, num1}]);
const Op2 = (func, num0, num1)             => Lazy(["Op2", {func, num0, num1}]);
const Ite = (cond, pair)                   => Lazy(["Ite", {cond, pair}]);
const Cpy = (name, numb, body)             => Lazy(["Cpy", {name, numb, body}]);
const Sig = (name, typ0, typ1, eras)       => Lazy(["Sig", {name, typ0, typ1, eras}]);
const Par = (val0, val1, eras)             => Lazy(["Par", {val0, val1, eras}]);
const Fst = (pair, eras)                   => Lazy(["Fst", {pair, eras}]);
const Snd = (pair, eras)                   => Lazy(["Snd", {pair, eras}]);
const Prj = (nam0, nam1, pair, body, eras) => Lazy(["Prj", {nam0, nam1, pair, body, eras}]);
const Eql = (val0, val1)                   => Lazy(["Eql", {val0, val1}]);
const Rfl = (expr)                         => Lazy(["Rfl", {expr}]);
const Sym = (prof)                         => Lazy(["Sym", {prof}]);
const Rwt = (prof, name, type, expr)       => Lazy(["Rwt", {prof, name, type, expr}]);
const Cst = (prof, val0, val1)             => Lazy(["Cst", {prof, val0, val1}]);
const Ann = (type, expr)                   => Lazy(["Ann", {type, expr}]);
const Ref = (name)                         => Lazy(["Ref", {name}]);
...
const is_at_level = (term, at_level, depth = 0, level = 0) => {
  var [ctor, term] = term();
  switch (ctor) {
...
const equal = (a_term, b_term) => {
  var [a_ctor, a_term] = a_term();
  var [b_ctor, b_term] = b_term();
