...
class AppPlay extends Component {
  ...
  async init_renderer() {
    ...
    this.intervals.renderer = setInterval(() => {
      ...
      if (this.app) {
        this.rendered = this.app.draw(this.get_app_state());
        if (this.rendered && this.rendered._ === "App.Render.vox") {
          var canvas = this.canvas;
          if (this.container && this.container.firstChild !== canvas) {
            this.container.innerHTML = "";
            this.container.appendChild(canvas);
          }
          var length = this.rendered.voxs.length;
          let capacity = this.rendered.voxs.capacity;
          var buffer = this.rendered.voxs.buffer;
  ...
  register_event(ev) {
    if (this.app) {
      this.execute_event(ev, this.app.init);
    }
