...
module.exports = class AppPlay extends Component {
  ...
  async fetch_data(url) {
    ...
    await res.text().then(str => this.data_fetched = str)
  }
  ...
  async init_input_events() {
   ...
   // Mouse movement event
    ...
    this.listeners.submit = (e) => {
      ...
      try {
        this.fetch_data(e.target.action)
      } catch (e) {
        this.data_fetched = "Error while fetching data from " + e.target.action
      }
      // FIXME: wait until get a response. I was having problems 
      // to use "await" here
      setTimeout(() =>
        this.register_event({
          ...
          response: this.data_fetched // IMPORTANT: only work for String
        }), 500
      )
      // this.register_event({
      //   _: "App.Event.onsubmit",
